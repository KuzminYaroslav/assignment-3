...
SELECT 
    ...
    -- Convert price from cents to currency units; handle NULLs for free items
    ...
    -- Keep genres as a list of structs for flexible analysis later
    g.app_details.data.genres AS genres,
    -- Developers is a simple string list
...
SELECT 
    r.appid,
    -- Access fields via the explicit 'review_item' column alias
    ...
    -- Safely cast vote score to double
...
FROM (
    -- 1. Unnest the root list
    SELECT unnest(reviews) as r 
    FROM raw_reviews_source
),
-- 2. Unnest the internal review list using explicit table/column aliasing
